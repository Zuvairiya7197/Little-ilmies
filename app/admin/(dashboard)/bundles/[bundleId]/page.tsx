...
export default async function EditBundlePage({ params }: PageProps) {
  ...
  const [bundle, products, pricingSettings] = await Promise.all([
    prisma.bundle.findUnique({
      where: { id: bundleId },
      include: { products: true, customPrices: true },
    }),
        ...
        defaultValues={{
          ...
          bundlePriceInr:
            bundle.bundlePriceInr != null
              ? bundle.bundlePriceInr / 100
              : undefined,
          bundlePriceUsd:
            bundle.bundlePriceUsd != null
              ? bundle.bundlePriceUsd / 100
              : undefined,
          productIds: bundle.products.map((p) => p.productId),
          sizePrices: Array.from(
            new Set(bundle.customPrices.map((price) => price.quantity)),
          )
            .sort((a, b) => a - b)
            .map((quantity) => {
              const rows = bundle.customPrices.filter(
              ...
              return {
                ...
                prices: Object.fromEntries(
                  rows.map((row) => [row.currencyCode, row.price / 100]),
                ),
